...
import { createInstrumentFlow, updateInstrumentFlow } from "@/lib/portfolio";
import { contributionInputSchema, contributionUpdateInputSchema } from "@/lib/schemas";
...
export async function PUT(request: Request) {
  try {
    const payload = await request.json();
    const parsed = contributionUpdateInputSchema.safeParse(payload);
    ...
    const id = updateInstrumentFlow(parsed.data, liveQuote);
    await refreshMarketData();
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "No se pudo actualizar la aportacion.",
      },
      { status: 500 },
    );
  }
